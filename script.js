...
/// inspects users array for patter names
/// find the name with highest index and
/// returns new name incremented by 1
function incUsername(pattern, users) {
  const reg = new RegExp(`\\bWeb-Dev\\b`);
  const idxArray = users
    .filter((user) => user.match(pattern))
    .map((user) => user.replace(reg, pattern + "0"))
    .map((user) => parseInt(user.replace(pattern, "")))
    .sort();
  console.log(idxArray);
  const [lastIdx] = idxArray.slice(-1);
  if (typeof lastIdx === "number") {
    return pattern + (lastIdx + 1).toString();
  } else {
    return pattern;
  }
}
